...
Gelişmiş asenkron mimarisi (QThread) sayesinde büyük log dosyalarını arayüzü dondurmadan işler.

## 🛠️ Kullanılan Teknolojiler
* **Geliştirme Dili:** Python 3.x
* **Arayüz (GUI) Kütüphanesi:** PySide6 (Hızlı, modern ve asenkron QThread destekli masaüstü arayüzü)
...
* **Tehdit İstihbaratı:** `requests` (VirusTotal API canlı sorguları için)
...
2. Gerekli Python kütüphanelerini kurun:
   ```bash 
   pip install PySide6 python-evtx requests
3. Proje dizininde terminali açarak uygulamayı başlatın:
   ```bash 
   python main.py

## 🔍 Kullanım
1. Uygulama açıldığında "Log Dosyası Yükle" butonuna tıklayın. Yeni bir dosya yüklerken, mevcut analizlerin üzerine ekleme (Append) yapabilir veya tabloyu sıfırlayabilirsiniz.
...
4. Tablo üzerindeki herhangi bir olaya çift tıklayarak olay detaylarını ve "Önerilen Aksiyon" penceresini görüntüleyin.

5. İşleminiz bittiğinde, sadece ekranda filtrelediğiniz şüpheli verileri "Analiz Raporunu İndir" butonu ile CSV veya JSON formatında dışa aktarın.
...
* Rule 3 (Şüpheli Yönetici Aktivitesi): "Administrator" kullanıcısına özel yetki ataması yapılması (Event ID 4672). (High - Turuncu)

* Rule 4 (Şüpheli İşlem - Process): Command Prompt (cmd.exe) gibi potansiyel zararlı script çalıştırabilecek processlerin başlatılması (Event ID 4688). (Critical - Kırmızı)
...
Proje kapsamında istenen ekstra özellikler ve endüstri standartlarındaki eklemeler projeye başarıyla entegre edilmiştir:

* Bonus 1 (.evtx Desteği): Uygulama python-evtx kütüphanesi kullanılarak doğrudan Windows'un şifreli .evtx formatındaki loglarını parse edebilir.

* Bonus 2 (Gerçek Zamanlı İzleme - Live Sync): Uygulama asenkron olarak (QThread) arka planda dosyayı dinler. Yeni bir log düştüğünde arayüz donmadan tablo ve dashboard sayıları otomatik güncellenir.

* Bonus 3 (Hibrit IOC Analizi & VirusTotal): Bilinen zararlı IP adresleri hem yerel veritabanından hem de VirusTotal API üzerinden anlık olarak sorgulanır. Zararlı IP giriş yaptığında sistem riskini ☠️ FATAL (Siyah) olarak atar ve arayüzde uyarı verir.
